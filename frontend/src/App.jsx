...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans">
      {/* Top Navigation Bar - Classic Enterprise Header */}
      <header className="bg-white border-b border-slate-300 px-6 py-3 flex justify-between items-center shadow-sm z-20 sticky top-0">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-lg font-bold text-slate-900 leading-tight">LogiMind</h1>
              <p className="text-xs text-slate-500 font-medium">Enterprise Supply Chain Platform</p>
            </div>
          </div>
        ...
        <div className="flex items-center gap-4">
          <div className="hidden lg:block text-right">
             <div className="text-xs font-semibold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200 inline-block">
               SYSTEM ONLINE
             </div>
          ...
          <button className="w-8 h-8 rounded-full bg-slate-100 border border-slate-300 flex items-center justify-center text-slate-600 hover:bg-slate-200">
      ...
      <main className="p-6 max-w-[1920px] mx-auto">
        <div className="grid grid-cols-12 gap-6">
          ...
          <section className="col-span-12 xl:col-span-3 lg:col-span-4 flex flex-col gap-4">
             <div className="bg-white border border-slate-300 rounded shadow-sm overflow-hidden flex flex-col h-[600px] lg:h-[calc(100vh-8rem)]">
          ...
          <section className="col-span-12 xl:col-span-6 lg:col-span-8 flex flex-col gap-6">
            ...
            {/* Main Map Panel */}
            <div className="bg-white border border-slate-300 rounded shadow-sm overflow-hidden flex-1 min-h-[400px]">
              <div className="bg-slate-50 px-4 py-3 border-b border-slate-300 flex justify-between items-center">
                 <h2 className="text-sm font-bold text-slate-700 uppercase tracking-wide">Global Logistics Map</h2>
                 <div className="flex gap-2">
                   <button className="text-xs bg-white border border-slate-300 px-2 py-1 rounded text-slate-600 hover:bg-slate-50">Filter View</button>
                   <button className="text-xs bg-white border border-slate-300 px-2 py-1 rounded text-slate-600 hover:bg-slate-50">Export Image</button>
                 </div>
              </div>
          ...
          <section className="col-span-12 xl:col-span-3 lg:col-span-12 flex flex-col">
             <div className="bg-white border border-slate-300 rounded shadow-sm overflow-hidden h-full">
...
const MetricCard = ({ label, value, subtext, trend, trendValue, highlight }) => {
  return (
    <div className={`bg-white border rounded shadow-sm p-4 flex flex-col justify-between ${highlight ? 'border-l-4 border-l-emerald-500' : 'border-slate-300'}`}>
      <div>
